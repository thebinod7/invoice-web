...
import { Invoice, formatCurrency, getStatusStyles } from './invoices'
...
export default function InvoiceCard({ invoice }: InvoiceCardProps) {
    ...
    const statusDisplay =
        statusKey === 'OVERDUE' ? 'Overdue' : invoice.status === 'PAID' ? 'Paid' : 'Sent'
    const styles = getStatusStyles(statusKey as 'PAID' | 'SENT' | 'OVERDUE')
                ...
                <div className="sm:text-right">
                    ...
                    <div
                        className={`text-xs mt-1 ${
                            invoice.isOverdue ? 'text-red-500' : 'text-slate-400'
                        }`}
                    >
                        {invoice.dueDate}
                    </div>
