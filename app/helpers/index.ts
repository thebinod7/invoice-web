...
import { SUPPORTED_CURRENCIES, SYMBOL_SUPPORTED_CURRENCIES } from '../constants/currency'
import { DEFAULT_CURRENCY } from '../constants'

const LIMIT_REACH_ERR_MSG = 'You have reached the limit for this feature! Please upgrade your plan.'
...
export const sanitizeError = (error: any) => {
    ...
    if (error.response.status === 409) {
        return 'Invoice number already exist. Please try new one!'
    }
...
export const formatCurrency = (amount: number, currencyCode = DEFAULT_CURRENCY) => {
    try {
        const options: {
            code: string
            symbol?: string
...
export const calculatePercentAmountOfTotal = (total: number, percentage: number) => {
